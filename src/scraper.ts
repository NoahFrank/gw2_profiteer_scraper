...
export class Scraper {
	...
	public run() {
		got
			.get('https://api.guildwars2.com/v2/commerce/prices', {json: true})
			.then(response => {
				...
				API.getItems(data)
					.then(response => {
						...
						if (response.statusCode === 200) {
							const items: ItemDetail[] = deserialize<ItemDetail[]>(
								ItemDetail,
								response.body
							);
							...
							console.log(
								`Got status: ${response.statusCode} | Message: ${response.statusMessage}`
							);
						}
					})
					.catch(error => {
